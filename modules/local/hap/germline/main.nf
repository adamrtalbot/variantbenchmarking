process HAP_GERMLINE {
    tag "$meta.id"
    label 'process_medium'

    // WARN: Version is hardcoded in the versions topic below. Please update it when bumping the hap-rs container.
    // hap-rs is not yet on crates.io; a published image is required for test_full.
    // Override with `params.haprs_container` (see conf/tests/test_haprs.config).
    container "${params.haprs_container}"
    ...
    tuple val("${task.process}"), val('hap-rs'), val('0.1.0'), topic: versions, emit: versions_haprs
